        ...
        for filename in fnmatch.filter(filenames, f"*{funscriptExtension}"):
            ...
            for j in videoExtensions:
                ...
                if os.path.exists(potentialVideoFile):
                    ...
                    break
            if videoFile != None:
                outputFileName = outputDir + os.sep + videoFile.replace(os.sep, '_')
                if not os.path.exists(outputFileName): warpAndRepeatClip(refFunscript, videoFile, i, outputFileName)
                videoFile = None
